...
def raioGraficalizador2(dados, intervalo, numeroIteracoes, taxaAprendizado):
    fig, ax = plt.subplots()
    x = [None] * dados.__len__()
...
for i in vetor1:
    for j in vetor2:
        for k in vetor3:
            data = constroi_modelo_2nn(X_treino, Y_treino, X_teste,
                                       Y_teste, i, j, 3, k, print_custo=False)
            raioGraficalizador(data['custos'], 10, i, j)
