...
def propaga(w, b, X, Y):
    ...
    # Propagação pra frente (no caso está usando a função sigmoide)
    Y_hat = sigmoide(np.dot(w.T, X)+b)
    termo1 = Y*np.log(Y_hat)
    termo2 = (1-Y)*np.log(1-Y_hat)

    # computa funcao custo J(w,b) --> operacao vetorial, Y.shape == Y_hat.shape
    ...
    # encontra o dw medio dentre os dw parciais (dw = 1/m * soma(x^(i)*(y_hat^(i) - y)))  --> operacao vetorial, dw tem dimensao (n_x,1)
    dw = 1/m * np.dot(X, (Y_hat-Y).T)
    # encontra o db medio dentre os db parciais (db = 1/m * soma(y_hat^(i) - y))  --> operacao vetorial, dw tem dimensao (n_x,1)
